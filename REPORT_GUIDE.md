...
**版本**: 2.0  |  **最后更新**: 2026-04-11  |  **维护**: Semiconductor Weekly Team
...
1. **新增板块 08** — 产品与体验设计（Product & Experience Design），关注 EDA UX、HMI、工业设计、品牌视觉等"设计维度"。板块总数从 11 增至 **12**。
2. **每个板块都补充了详细说明** — 四个维度：定位 / 内容要素 / 选稿原则 / 常见陷阱。
3. **移动端适配独立设计** — 不只是缩放，而是重新设计的排版（汉堡菜单 + 横向 TOC chips + 紧凑股价 2×2）。
4. **数据模型完整化** — `designTrends` 字段纳入规范。
5. **生成脚本更新** — PPT 从 12 页扩到 13 页，Word 从 11 板块扩到 12 板块。
6. **经验沉淀补充** — 新增 "常见陷阱" 附录，把首期踩过的坑全部记录。
...
网页版必须提供三个格式的下载入口（顶部导航按钮 + 移动端汉堡菜单）。
...
| # | 板块 | 中文标题 | 英文副标题 | 核心内容 |
|---|---|---|---|---|
| 01 | 本周速览 | 本周速览 | Weekly Digest | 5 条一句话摘要 |
| 02 | 市场脉搏 | 市场脉搏 | Market Pulse | SOX + 4 支股票卡 |
| 03 | 公司动态 | 设备巨头动态 | Equipment Giants | 4-5 家核心公司 |
| 04 | 前沿技术 | 前沿技术 | Tech Frontier | 3 条技术进展 |
| 05 | 中国板块 | 中国半导体动态 | China Semiconductor | 3-4 家中国公司 + 政策展望 |
| 06 | 学术研究 | 学术与研究前沿 | Academic & Research | 4 条学术突破 |
| 07 | AI 与智能化 | AI 与智能化 | AI & Smart Manufacturing | 4 条 + Capital Flow box |
| 08 | 产品与体验设计 | 产品与体验设计 | Product & Experience Design | 4-5 条设计趋势 + UX Shift box |
| 09 | 政策地缘 | 政策与地缘 | Policy & Geopolitics | 3 条政策 |
| 10 | 产业格局 | 产业格局 | Industry Landscape | 1 个 highlight box |
| 11 | 值得一读 | 值得一读 | Worth Reading | 3-4 篇推荐文章 |
| 12 | 关键日程 | 关键日程 | Upcoming Events | 6-10 条时间表 |

### 内容约束

- **总字数**：中文 3,500-5,000 字
- **阅读时间**：6-10 分钟
- **PDF 页数**：10-13 页 A4
- **PPT 页数**：13 页 16:9
- **单条文字**：不超过 100 字（避免长段落）
...
每个板块都遵循**四个维度**的描述：**定位 → 内容要素 → 选稿原则 → 常见陷阱**。

---

#### 板块 01 — 本周速览（Weekly Digest）

**定位**：报告的"头版导读"。读者扫一眼速览就能了解本周最重要的 5 件事，决定是否继续往下读。

**内容要素**：
- 严格 **5 条**（不多不少），每条一句话（≤ 40 字）
- 每条必须有**编号**（01–05）和**标签**（POLICY / AMAT / SEMI / CHINA / HIGH-NA 等）
- 每条后面必须有**至少 1 个来源链接**
- 关键数据加粗（`<strong>`）

**选稿原则**：
- 必须是本周**发生的事件**（不是观点、不是回顾）
- 标签要覆盖不同维度（避免 5 条全是同一家公司）
- **重要性排序**：政策冲击 > 头部公司动作 > 行业数据 > 中国动态 > 技术里程碑
- 如果本周有"头条级"事件，速览第一条和 Hero 标题应呼应

**常见陷阱**：
- ❌ 5 条全是同一家公司（应分散视角）
- ❌ 一句话写成长段落（严格控制 40 字）
- ❌ 标签和内容不匹配（如 `POLICY` 却是公司财报）

---

#### 板块 02 — 市场脉搏（Market Pulse）

**定位**：快速呈现资本市场对本周事件的反应，不是投资建议。

**内容要素**：
- **1 个行业指数条**：SOX（费城半导体指数），包含当前值、52W 高点、离峰值距离
- **4 张股票卡片**：AMAT、LRCX、ASML、KLAC（固定 4 家核心设备股）
  - Ticker（等宽字体）
  - 公司名
  - 当前股价
  - 涨跌幅（▲绿/▼红）+ 日期
...
**选稿原则**：
- 股价以**本周最后一个交易日**为准
- 涨跌幅是**当日**或**YTD**，必须在备注里注明
- 备注必须和本周事件关联（如"MATCH Act 冲击"）
- 如果某家公司出现超过 ±5% 变动，必须在备注里解释

**常见陷阱**：
- ❌ 使用过时股价（必须和 dateRange 匹配）
- ❌ 备注写成空话（"市场情绪波动"无信息量）
- ❌ 涨跌方向和颜色不一致

---

#### 板块 03 — 设备巨头动态（Equipment Giants）

**定位**：报告的**主力板块**，深度呈现核心公司本周的实质动作。

**内容要素**：
- **4-5 家核心公司**：Applied Materials、Lam Research、ASML、Tokyo Electron、KLA
  - **公司名** + **ticker**（同行，两端对齐）
  - **Highlight 区**（红色左边框 + 淡红渐变底）：本周最重要一句话
  - **事件列表**：2-4 条，每条格式 `[日期/标签]  内容`
- 每条事件末尾必须有来源链接

**选稿原则**：
- 每家公司至少有 1 条本周事件，没有就跳过该公司
- **Highlight 选最重磅的那一条**（通常是新品发布或财报超预期）
- 事件优先级：新品发布 > 财报 > 订单 > 分析师评级 > 股东事件
- TEL 和 KLA 可以合并为一个 company block（如果事件较少）

**常见陷阱**：
- ❌ Highlight 写成两句话（必须一句话）
- ❌ 事件日期错乱（必须按时间倒序或重要性排序）
- ❌ 把"展望"类信息当作本周事件（应明确标注为"展望"）

---

#### 板块 04 — 前沿技术（Tech Frontier）

**定位**：关注**工艺/架构/材料**层面的行业进展，面向懂技术的领导。

**内容要素**：
- **3 条**技术进展（严格）
- 每条格式：**标题（12-20 字）** + **说明段（60-100 字）**
- 来源链接 1-2 个

**选稿原则**：
- 涵盖范围：先进制程（GAA、CFET、BSPD）、EUV/光刻、新材料（Ga2O3、CNT）、封装（HBM、混合键合）
- 优先选**进入量产节点**的进展（比停留在实验室更有新闻价值）
- 避开纯学术论文（那属于"学术研究"板块）
- 标题要有动词和数据（"TSMC N2P 进入量产倒计时"）

**常见陷阱**：
- ❌ 和"AI 与智能化"板块混淆（AI 芯片设计归 AI 板块，芯片工艺归前沿技术）
- ❌ 写成技术科普（应聚焦"本周发生了什么"）
- ❌ 超过 3 条（必须克制）

---

#### 板块 05 — 中国半导体动态（China Semiconductor）

**定位**：独立的中国板块，避免中国新闻散落在各处。服务于对国产替代敏感的读者。

**内容要素**：
- **3-4 家中国公司**：北方华创、中微公司、新凯来、盛合晶微、拓荆、精测电子 等
  - 公司名 + ticker（A 股代码）
  - 可选 Highlight 区
  - 事件列表
- **1 个 Policy & Outlook highlight box**：政策/市场展望

**选稿原则**：
- 优先**真实订单**和**真实产品发布**，避开 PR 稿和目标性宣言
- 自主化进展必须有**可验证的证据**（如"已在 SMIC 7nm 产线测试"）
- Outlook box 必须含**具体数字**（自给率、订单目标、WFE 增速等）
- SEMICON China 展会是中国公司发布新品的集中时点

**常见陷阱**：
- ❌ 过度渲染"突破"字眼（保持中立客观）
- ❌ 没有国际对标（读者需要知道相对位置）
- ❌ 只有公司没有政策（Outlook box 必须有）

---

#### 板块 06 — 学术与研究前沿（Academic & Research）

**定位**：聚焦顶刊/顶会/顶级实验室的研究突破，面向技术信仰度高的读者。

**内容要素**：
- **4 条**学术突破
- 每条格式：**标题** + **说明段（60-120 字）** + **来源（Nature / IEEE / imec 官网等）**

**选稿原则**：
- 信源优先级：**Nature / Nature Electronics / Nature Communications > IEDM / VLSI / IRPS / ISSCC > imec / CEA-Leti / IMEC 博客**
- 必须有**可引用的论文链接或机构新闻稿**
- 优先选**有工业应用前景**的研究（纯理论放弃）
- 时间窗口可以放宽到近 30 天
- 涵盖领域：量子计算、新型器件、3D 存储、新材料、光子芯片

**常见陷阱**：
- ❌ 只写论文结论不写机构（必须标注 imec、TSMC 联合等）
- ❌ 选太前沿以至于读者无法理解（要选能用 1 句话解释的）
- ❌ 和"前沿技术"板块混淆（前沿技术看量产，学术看探索）

---

#### 板块 07 — AI 与智能化（AI & Smart Manufacturing）

**定位**：两个方向——**AI 改造半导体制造**（AI in fab）和 **半导体为 AI 服务**（AI chips）。

**内容要素**：
- **4 条**动态 + **1 个 Capital Flow highlight box**
- 覆盖方向：
  - AI 缺陷检测、良率优化、数字孪生
  - EDA Agentic 工具（Cadence ChipStack、Synopsys AgentEngineer）
  - AI 芯片新品（HBM、推理加速器、定制硅）
  - 车规 RISC-V、边缘 AI

**选稿原则**：
- AI 制造 和 AI 芯片 各占 1-2 条，保持平衡
- **Capital Flow box** 关注资本/战略层面（超算 CapEx、并购、合作）
- 数字越具体越好（"98.5% die 级精度"优于"提升显著"）

**常见陷阱**：
- ❌ AI 芯片和"设备巨头动态"重复（设备厂 AI 工艺归 AI 板块，AI 芯片设计公司归 AI 板块）
- ❌ 只讲产品不讲应用场景

---

#### 板块 08 — 产品与体验设计（Product & Experience Design）

**定位**：跨领域观察板块，关注**半导体产业链里所有与"人"相关的设计维度**。这是本报告区别于其他行业周报的特色板块。
...
- 可附 **1 个 UX Shift highlight box**：总结本周的交互范式变化

**选稿原则**：
- 每条必须是**真实发生的产品/事件**，不是观点评论
- 优先选有**视觉变革**或**交互范式转变**意义的事件
- 避免纯技术性能讨论（那属于"前沿技术"板块）
- 如果本周没有显著事件，可扩展到**近 30 天**的趋势
- 理想的一条应该能让设计师读完后想点开图看

**常见陷阱**：
- ❌ 把"AI 芯片发布"当成设计新闻（应关注其**物理形态/视觉语言**）
- ❌ 被 PR 稿带节奏（公司自吹的"美学"不算）
- ❌ 过度技术化（这是给产品和设计视角的板块）

---

#### 板块 09 — 政策与地缘（Policy & Geopolitics）

**定位**：跟踪影响半导体产业链的宏观政策和地缘博弈。这个板块决定了读者对**未来不确定性**的判断。

**内容要素**：
- **3 条**政策，每条带国别 tag：
  - `U.S. Congress` / `U.S. Tariff`（蓝色）
  - `中国` / `CN`（红色）
  - `CHIPS Act` / `Global` / `EU`（紫色）
- 每条格式：**tag** + **标题** + **说明段**
- 说明段必须包含：**法案名/政策编号**、**关键数据影响**、**时间节点**

**选稿原则**：
- 追溯到**本年度的重大立法/管制**，不仅限于本周
- 必须引用**官方源**或**一流媒体**（Bloomberg、Reuters、路透、SCMP）
- 如果 JPMorgan / Morgan Stanley 等投行给出量化影响，必须引用
- 避免预测性措辞（"可能"、"或将"），只陈述已发生的事
...
#### 板块 10 — 产业格局（Industry Landscape）

**定位**：宏观供应链、扩产、并购层面的格局变化。这个板块通常只有 **1 个 Highlight box**。

**内容要素**：
- **1 个 highlight box**（`Fab Expansion` 或 `M&A` 或 `Supply Chain`）
- 内容覆盖：
  - 新建晶圆厂项目（地点、规模、投产时间）
  - 公司并购和战略合作
  - 供应链断链/恢复
...
**选稿原则**：
- 优先**全球数据**（SEMI 报告、IC Insights）
- 关注 TSMC 亚利桑那、日本熊本、Intel 俄亥俄、三星泰勒等重点项目
- 如果本周没有扩产新闻，可以放**并购或供应链**内容

**常见陷阱**：
- ❌ 写成多条 item（应保持单个 highlight box）
- ❌ 没有数字（必须有金额或数量）

---

#### 板块 11 — 值得一读（Worth Reading）

**定位**：编辑推荐，为深度阅读者准备。每条应该让读者想点开原文。

**内容要素**：
- **3-4 篇**文章
- 每条格式：
  - **编号（大号灰色数字）** + **标题（可点击跳转原文）**
  - **来源 + 日期**（mono 灰色）
  - **一句话推荐语（20-40 字）**

**选稿原则**：
- 必须是**深度长文**或**权威报告**，不是新闻快讯
- 来源优先级：SEMI 报告 > SemiEngineering 深度 > EE Times 专题 > Bloomberg 长文
- 推荐语说明"为什么值得读"（不是复述内容）
- 4 篇应覆盖不同维度：工艺、政策、学术、资本
...
#### 板块 12 — 关键日程（Upcoming Events）

**定位**：给领导一个"日历"——知道接下来什么时候关注什么事件。

**内容要素**：
- **6-10 条**日程，日期 + 事件的双列结构
- 涵盖：
  - 核心公司**财报发布**（ASML、Lam、AMAT、KLA、TEL）
  - **股东大会**、**股息发放**
  - **行业会议**（IEDM、VLSI、ISSCC、SEMICON 系列）
  - **学术会议**（VLSI Symposium、SPIE）

**选稿原则**：
- 时间窗口：未来 **3-8 周**
- 按时间顺序排列
- 事件名称使用**公司缩写** + **类型**（如"ASML Q1 财报"）
- 不包含"预计"、"可能"等不确定事件
...
所有内容必须结构化为 ES Module export。完整模型：
...
export const chinaSection = {
  companies: [
    { name, ticker, highlight, events: [[date, text], ...] },
    // 3-4 家
  ],
  outlook: '政策与市场展望文字...',
};
...
export const policy = [
  { tag: 'U.S. Congress', title, text },
  // 3 条
];

export const landscape = '一段话描述产业格局...';

export const reading = [
  { title, source, desc, url },
  // 3-4 条
];
...
6. **设计类**（v2.0 新增）：Cadence/Synopsys 官方博客、SIGGRAPH、CHI、Core77、Designboom

**禁止使用**：匿名博客、未署名文章、无日期的聚合页。

### 采集流程（并行 Agent）

周报采集分成 **4 个并行子 Agent**（v2.0 新增 Agent D），减少主 context 消耗：
...
#### Highlight Box（产业格局/Outlook/UX Shift）
...
<nav> 顶部导航（sticky）
  ├── 品牌名
  ├── 日期
  └── PDF / PPT / Word 下载按钮（桌面） / 汉堡菜单（移动）
<body>
  <cover-zone> 封面区（白底）
    ├── Hero（label + h1 + subtitle + meta chips）
    └── TOC（目录 — 桌面网格 / 移动横向 chips）
  <content-zone> 正文区（暖灰底 #fafaf8）
    ├── 12 个 section
    └── site-footer
<mobile-menu> 移动抽屉菜单（隐藏在顶部）
```
...
- **目录**桌面是网格卡片，移动是横向滑动 chips
- **股价卡**桌面端 2×2 grid，移动端 2×2 紧凑 grid（不是单列）
- **公司块**左侧红色边框 + 淡红渐变作为 highlight 区
- **板块 08 设计**和其他板块视觉一致，不做特殊化
...
- **16:9** (13.33 × 7.5 英寸)
- 使用 `pptxgenjs` 生成
- **13 页**（封面 + 12 个板块，但板块 04 跨页、板块 03 独立两页等可能调整）
- 注意：设备巨头板块通常占 2 页
...
```
Step 1: 调研（并行 4 个 Agent — A/B/C/D）
    ↓
Step 2: 整理数据到 data.mjs（12 个 section）
    ↓
Step 3: 生成四个版本
    ├── 更新 index.html（内容区填充）
...
// 13 slides，复用 data.mjs
```

**Word** (`generate-docx.mjs`):
```javascript
import { Document, Packer, ... } from 'docx';
import { meta, digest, ..., designTrends, ... } from './data.mjs';
```
...
### 内容
- [ ] 12 个板块齐全，顺序正确（注意 v2.0 加入了 08 设计板块）
- [ ] 每条信息都有来源链接（最少 1 个）
- [ ] 加粗的关键数据不超过总字数 15%
...
- [ ] 打开文件无报错
- [ ] 字体正确渲染（中文不乱码）
- [ ] 页码准确（PPT = 13 / Word 由页面决定）
- [ ] 板块编号（01-12）和 `data.mjs` 对齐
...
6. **OAuth 缺 workflow scope**：推送 `.github/workflows/*.yml` 文件需要 GitHub token 有 `workflow` scope。用 `gh auth refresh -h github.com -s workflow` 刷新。

7. **移动端底部栏冗余**：如果顶部已有汉堡菜单，不要再加底部固定下载栏，会打断阅读。

8. **关键日程不要堆叠**：移动端也保持日期 | 事件的双列结构，更可读。

9. **板块编号不对齐**：加新板块后必须**同步更新 index.html、PPT、Word 的板块编号**。最后 4 个板块（政策、产业、值得一读、日程）的编号都会往后挪。

10. **TOC 编号要和 section-num 对齐**：TOC 和板块内部的编号必须一致（不要 TOC 用 12 而 section 用 11）。

11. **SOX 指数条错位**：移动端 SOX 数字和标签必须 `flex-wrap: nowrap` + 右对齐，否则数字会换行错位。

12. **股价卡移动端不要单列**：2×2 比单列更紧凑、更适合快速扫读。

13. **数据源单一**：所有版本必须从 `data.mjs` 读取，避免在不同脚本里硬编码文案。
...
- [ ] **PDF Figma 排版**：手动在 Figma 里调出最终 PDF 样式，然后导出为 `report-YYYY-WNN.pdf`
- [ ] **自动化 Agent**：用 Claude Scheduled Tasks 每周一自动跑研究 → 生成 → 推送流水线
- [ ] **订阅邮件**：Gmail SMTP 自动发送周报到订阅列表
- [ ] **历史归档页**：列出所有历史期数，支持切换浏览
- [ ] **Feed 订阅**：提供 RSS 供其他 Agent 消费
