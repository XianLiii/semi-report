// ============================================================
// Semiconductor Weekly V2 — 数据源
// V2 变化：
//   1. 每条信息增加 `why` 字段（为什么重要 / 工程师视角解读）
//   2. 设备巨头：5 家独立（AMAT/Lam/ASML/TEL/KLA），产品级深度
//   3. 新增"综合判断"板块
//   4. 内容量不限制，对标专业情报日报
//   5. 增加 DRAM 路线图、HBM 演进、GAA 工艺链等结构化数据
// ============================================================

export const meta = {
  title: 'MATCH Act 冲击波，设备股剧烈震荡',
  subtitle: '美国国会推出新芯片设备出口管制法案，ASML 单日跌近 5%；AMAT 发布两款 GAA 2nm 沉积系统，股价反弹 8%；五大设备厂商密集发布下一代工艺装备。',
  brand: 'Semiconductor Weekly',
  vol: 'Vol.01 / No.15',
  dateRange: '2026.04.07 — 04.09',
  confidential: 'Confidential',
  version: 'V2',
};

// ============================================================
// 01 本周速览
// ============================================================
export const digest = [
  {
    tag: 'POLICY',
    text: '美国众议院推出 MATCH Act，拟封堵 ASML DUV 对华出口通道，ASML 单日跌 4.7%',
    why: '直接影响中国 Fab 的 DUV 光刻机获取路径，可能加速国产替代节奏',
    sources: [{ name: 'Bloomberg', url: 'https://www.bloomberg.com/news/articles/2026-04-02/us-lawmakers-propose-crackdown-on-sales-of-chip-tools-to-china' }],
  },
  {
    tag: 'AMAT',
    text: 'Applied Materials 发布 Precision Selective Nitride PECVD 和 Trillium ALD 两款沉积系统，面向 GAA 2nm',
    why: 'STI 选择性沉积和亚纳米膜厚控制是 GAA 量产的两大工艺瓶颈，这两款工具直接解决',
    sources: [{ name: 'StockTitan', url: 'https://www.stocktitan.net/news/AMAT/applied-materials-introduces-deposition-systems-for-angstrom-era-7v0938tplllx.html' }],
  },
  {
    tag: 'LAM',
    text: 'Lam Research 发布 Akara 导体刻蚀系统和 ALTUS Halo 钼 ALD，均已被领先厂商选为量产机台',
    why: 'Akara 的 DirectDrive 固态等离子体源是刻蚀领域 20 年来最大架构变革；钼替代钨是金属化路线的范式转移',
    sources: [{ name: 'Lam Newsroom', url: 'https://newsroom.lamresearch.com/everything-about-akara' }],
  },
  {
    tag: 'TEL',
    text: 'TEL LITHIUS Pro DICE 开始在纽约 Albany NanoTech 安装，北美首个公共 High-NA EUV 中心成型',
    why: '美国本土 High-NA EUV 生态落地的关键一步，配套 ASML EXE:5200 将在年底前 first light',
    sources: [{ name: 'Governor NY', url: 'https://www.governor.ny.gov/news/governor-hochul-announces-ny-creates-begins-installation-first-major-tool-high-na-euv' }],
  },
  {
    tag: 'SEMI',
    text: 'SEMI 发布数据：2025 全球半导体设备销售 $1,351 亿，预计 2027 年达 $1,560 亿创新高',
    why: 'AI 驱动的先进逻辑和存储投资是主要增长引擎，台湾 2025 设备支出暴涨 90%',
    sources: [{ name: 'SEMI', url: 'https://www.semi.org/en/semi-press-release/semi-projects-double-digit-growth-in-global-300mm-fab-equipment-spending-for-2026-and-2027' }],
  },
  {
    tag: 'HIGH-NA',
    text: 'Imec 接收 ASML EXE:5200 High-NA EUV 系统，全球不到 12 台，预计 Q4 完成验证',
    why: 'Imec 是全球 sub-2nm 工艺的核心研发枢纽，验证结果将直接影响 TSMC/Intel/Samsung 的量产节奏',
    sources: [{ name: 'Imec', url: 'https://www.imec-int.com/en/press/imec-receives-worlds-most-advanced-high-na-euv-system' }],
  },
];

// ============================================================
// 02 市场脉搏
// ============================================================
export const market = {
  sox: { value: '7,877', high52w: '8,498', offPeak: '-7.3%' },
  stocks: [
    { ticker: 'AMAT', name: 'Applied Materials', price: '$352.62', change: '+8.0%', date: 'Apr 8', up: true, note: 'GAA 新品发布提振' },
    { ticker: 'LRCX', name: 'Lam Research', price: '$235.00', change: '+9.5%', date: 'Apr 8', up: true, note: 'YTD +37.6%，Akara 订单强劲' },
    { ticker: 'ASML', name: 'ASML Holding', price: '$1,317', change: '-4.7%', date: 'Apr 7', up: false, note: 'MATCH Act 冲击，中国营收占 29%' },
    { ticker: 'KLAC', name: 'KLA Corporation', price: '$1,661', change: '+7.4%', date: 'Apr 8', up: true, note: 'YTD +30.3%，$70 亿回购计划' },
    { ticker: '8035.T', name: 'Tokyo Electron', price: '¥24,800', change: '+3.2%', date: 'Apr 8', up: true, note: 'DICE 安装 Albany，资本支出创新高' },
  ],
};

// ============================================================
// 03 设备巨头动态 — 5 家独立，产品级深度
// ============================================================
export const equipment = [
  {
    name: 'Applied Materials',
    ticker: 'NASDAQ: AMAT',
    highlight: '4 月 8 日发布 Precision Selective Nitride PECVD 和 Trillium ALD 两款沉积系统，面向 GAA 2nm 及以下节点的原子级精度制造。',
    why: 'Precision PECVD 实现 STI 工艺中氮化硅的选择性沉积（仅在目标表面生长），解决 GAA 纳米片释放后的隔离难题；Trillium ALD 提供亚纳米膜厚控制和高共形覆盖，是栅极堆栈沉积的关键。两款工具与 AMAT 的 Viva、Sym3 Z、Endura 等形成完整的 GAA 沉积解决方案。',
    highlightSources: [{ name: 'StockTitan', url: 'https://www.stocktitan.net/news/AMAT/applied-materials-introduces-deposition-systems-for-angstrom-era-7v0938tplllx.html' }],
    events: [
      ['Apr 8', '新品发布后股价当日大涨约 8%，市场反应积极', [{ name: 'SeekingAlpha', url: 'https://seekingalpha.com/news/4573357-applied-materials-surges-as-it-unveils-new-chipmaking-systems' }]],
      ['Q1', 'Q1 FY2026 EPS $2.38（超预期 $0.17），营收 $70.1 亿', [{ name: 'TrendForce', url: 'https://www.trendforce.com/news/2026/02/13/news-applied-materials-guides-april-quarter-above-estimates-sees-20-semi-growth-in-2026-dram-leads/' }]],
      ['展望', '管理层预计 2026 年半导体行业增长 20%+，DRAM 领涨', []],
    ],
  },
  {
    name: 'Lam Research',
    ticker: 'NASDAQ: LRCX',
    highlight: '2 月发布 Akara 导体刻蚀系统和 ALTUS Halo 钼 ALD，均已被领先 DRAM 和代工厂商选为量产机台。1 月 Aether 干法光刻胶被领先存储厂商采纳为量产工具。',
    why: 'Akara 的 DirectDrive 是业界首个固态等离子体源，响应速度比前代快 100 倍，实现埃米级 CD 均匀性。ALTUS Halo 是全球首款钼 ALD 工具，电阻比传统钨降低 50% 以上。Aether 干法光刻胶化学品用量比湿法减少 5-10 倍。三款产品分别对应刻蚀、金属化、光刻三大工艺方向的范式转移。',
    highlightSources: [
      { name: 'Lam Newsroom', url: 'https://newsroom.lamresearch.com/everything-about-akara' },
      { name: 'Lam IR', url: 'https://investor.lamresearch.com/2025-02-19-Lam-Research-Ushers-in-New-Era-of-Semiconductor-Metallization-with-ALTUS-R-Halo-for-Molybdenum-Atomic-Layer-Deposition' },
    ],
    events: [
      ['Akara', 'DirectDrive 固态等离子体源（100x 响应速度）+ TEMPO 脉冲 + SNAP 离子能量控制', [{ name: 'Semiconductor Digest', url: 'https://www.semiconductor-digest.com/lam-research-unveils-industrys-most-advanced-conductor-etch-technology-to-date/' }]],
      ['Halo', 'ALTUS Halo 钼 ALD：电阻比钨降低 50%+，正在三星/SK 产线认证', [{ name: 'Lam IR', url: 'https://investor.lamresearch.com/2025-02-19-Lam-Research-Ushers-in-New-Era-of-Semiconductor-Metallization-with-ALTUS-R-Halo-for-Molybdenum-Atomic-Layer-Deposition' }]],
      ['Aether', 'Aether 干法光刻胶被领先存储厂商采纳为 EUV 量产工具，与 JSR 交叉授权', [{ name: 'Lam IR', url: 'https://investor.lamresearch.com/2025-01-29-Breakthrough-EUV-Dry-Photoresist-Technology-from-Lam-Research-Adopted-by-Leading-Memory-Manufacturer' }]],
      ['财报', 'Q2 FY2026 营收 $53.4 亿（同比 +22%），Q3 指引 $57 亿，连续四季超预期', [{ name: 'Zacks', url: 'https://www.zacks.com/commentary/2895120/bull-of-the-day-lam-research-lrcx' }]],
    ],
  },
  {
    name: 'ASML Holding',
    ticker: 'EURONEXT: ASML',
    highlight: 'MATCH Act 冲击下股价单日跌 4.7%，但随后因美伊停火反弹 6.35%。High-NA EUV 进入商用部署阶段，全球不到 12 台 EXE:5200 已交付。',
    why: 'ASML 是全球唯一 EUV 光刻机供应商。MATCH Act 若通过，将把管制范围从 EUV 扩展到 DUV（中国 Fab 目前仍可获取的光刻设备），直接影响 ASML 约 29% 的营收。High-NA EUV（NA 0.55）分辨率从 13nm 提升到 8nm，是 sub-2nm 节点的唯一光刻选项。',
    highlightSources: [{ name: 'CNBC', url: 'https://www.cnbc.com/2026/04/07/asml-shares-today-us-chip-export-curbs-china.html' }],
    events: [
      ['High-NA', 'EXE:5200B 参数：NA 0.55，分辨率 8nm，单价 $3.5-4 亿，175 WPH，0.7nm overlay', [{ name: 'ASML', url: 'https://www.asml.com/en/products/euv-lithography-systems/twinscan-exe-5200b' }]],
      ['SK Hynix', '2025.09 SK Hynix 安装 EXE:5200B 于利川 M16 厂，DRAM 制造首次引入 High-NA EUV', [{ name: 'SK Hynix', url: 'https://news.skhynix.com/sk-hynix-introduces-industrys-first-commercial-high-na-euv/' }]],
      ['订单', 'SK Hynix 承诺 $80 亿采购 ~30 台 EUV；Samsung 计划 $40 亿采购 ~20 台', [{ name: '24/7 Wall St', url: 'https://247wallst.com/investing/2026/04/06/asml-revenue-is-about-to-explode-higher-again/' }]],
      ['Apr 15', 'Q1 财报发布 | 指引：2026 净销售额 340-390 亿欧元', []],
    ],
  },
  {
    name: 'Tokyo Electron',
    ticker: 'TSE: 8035',
    highlight: 'LITHIUS Pro DICE 开始在纽约 Albany NanoTech 安装（4 月 9 日），同期 EVAROS 批量热处理系统和 Epsira DX 数字化平台发布。',
    why: 'LITHIUS Pro DICE 是专为 High-NA EUV 设计的涂布/显影机，缺陷率比前代降低 50%+，是 ASML EXE:5200 的关键配套设备。Albany 安装标志着美国本土 High-NA 生态的正式落地。EVAROS 一次处理 200 片晶圆，是先进 ALD 工艺的产能瓶颈解决方案。',
    highlightSources: [
      { name: 'TEL Newsroom', url: 'https://www.tel.com/news/product/2025/20251215_002.html' },
      { name: 'Governor NY', url: 'https://www.governor.ny.gov/news/governor-hochul-announces-ny-creates-begins-installation-first-major-tool-high-na-euv' },
    ],
    events: [
      ['DICE', 'LITHIUS Pro DICE：缺陷降 50%+，行业最快涂布/显影速度，单位面积产能提升 25%+', [{ name: 'TEL', url: 'https://www.tel.com/news/product/2025/20251215_002.html' }]],
      ['EVAROS', 'EVAROS：一次 200 片，CO2 排放降 25%，大口径排气管路适配 ALD', [{ name: 'TEL', url: 'https://www.tel.com/news/product/2025/20251215_001.html' }]],
      ['Acrevia', 'Acrevia 气体簇束系统：减少 EUV 光刻的随机缺陷和线边缘粗糙度', [{ name: 'TEL', url: 'https://www.tel.com/news/product/2024/20240708_002.html' }]],
      ['LEXIA-EX', 'LEXIA-EX PVD：吞吐量提升 20%，占地面积缩小 40%', [{ name: 'TEL', url: 'https://www.tel.com/news/product/2024/20241209_002.html' }]],
      ['Ulucus LX', 'Ulucus LX 激光剥离系统：DI 水用量降 90%+，支持晶圆重复利用', [{ name: 'TEL', url: 'https://www.tel.com/news/product/2024/20241209_001.html' }]],
      ['资本', '先进芯片设备占比提升至 40%，FY2026 资本支出增 48% 至 2400 亿日元创新高', [{ name: 'TrendForce', url: 'https://www.trendforce.com/news/2026/01/09/news-tokyo-electron-reportedly-raises-fy26-capex-48-to-record-high-bets-on-dram-etching-demand/' }]],
    ],
  },
  {
    name: 'KLA Corporation',
    ticker: 'NASDAQ: KLAC',
    highlight: '2024-2025 年密集发布先进封装和量测新品矩阵：Lumina 基板检测、Kronos 1190 晶圆级封装检测、AXION T2000 X 射线量测。',
    why: 'GAA 比 FinFET 增加约 30% 薄膜量测层和 50% 关键检测层。先进封装（HBM、CoWoS、玻璃基板）是 KLA 新增长极。Lumina 是业界首款覆盖玻璃芯基板的检测系统，AXION T2000 用 CD-SAXS X 射线测量 3D NAND 通道孔——这些都是传统光学方法无法完成的。',
    highlightSources: [
      { name: 'KLA', url: 'https://www.kla.com/advance/innovation/advancing-connectivity-with-kla-new-ic-substrate-product-portfolio' },
    ],
    events: [
      ['Lumina', 'Lumina IC 基板检测系统：MMI 多模态成像 + AI 分类，覆盖玻璃芯基板', [{ name: 'KLA', url: 'https://www.prnewswire.com/news-releases/kla-unveils-comprehensive-ic-substrate-portfolio-for-a-new-era-of-advanced-semiconductor-packaging-302275682.html' }]],
      ['Kronos', 'Kronos 1190：150nm 灵敏度，DefectWise AI，覆盖 TSV/RDL/Cu pillar', [{ name: 'KLA IR', url: 'https://ir.kla.com/news-events/press-releases/detail/9/kla-announces-enhanced-portfolio-of-systems-for-advanced' }]],
      ['AXION', 'AXION T2000 X 射线量测：CD-SAXS 技术，测量 3D NAND 通道孔形状/倾斜/弯曲', [{ name: 'KLA', url: 'https://ir.kla.com/news-events/press-releases/detail/447/kla-launches-revolutionary-x-ray-metrology-system' }]],
      ['资本', '宣布 $70 亿股票回购计划，连续第 17 年提高股息', [{ name: 'StockStory', url: 'https://stockstory.org/us/stocks/nasdaq/klac/news/why-up-down/why-is-kla-corporation-klac-stock-soaring-today-3' }]],
    ],
  },
];

// ============================================================
// 04 前沿技术
// ============================================================
export const techFrontier = [
  {
    title: 'GAA 与背面供电进入量产倒计时',
    text: 'TSMC N2P（2nm + 背面供电）计划 2026 下半年投产；A16（1.6nm + Super Power Rail）紧随其后。背面供电将整个供电网络移到晶圆背面，需要将硅片研磨到 <100nm 并制作 ~100nm 直径的 nano-TSV（传统 TSV 约 10um）。',
    why: '背面供电消除了正面金属层中的供电线路，减少电压降和噪声，但引入了极薄晶圆翘曲、正面-背面对准精度、热应力等新挑战。模拟显示局部热点温度可能升高 14°C。这对研磨/键合/量测设备（TEL Ulucus、Besi、KLA）是新的增量市场。',
    sources: [
      { name: 'SemiEngineering', url: 'https://semiengineering.com/backside-power-delivery-nears-production/' },
      { name: 'Lam Research', url: 'https://newsroom.lamresearch.com/transistor-channel-stress-backside-power-delivery-networks' },
    ],
  },
  {
    title: 'High-NA EUV 部署时间线与技术参数',
    text: 'ASML EXE:5200B：NA 0.55（比 NXE 提升 40%），分辨率 8nm（vs 13nm），175 WPH，单价 $3.5-4 亿。2023.12 首台交付 Intel → 2025.09 SK Hynix M16（DRAM 首用）→ 2025 年底 Samsung 2 台 → 2026.03 Imec → 全球不到 12 台。2026 预计交付 5-10 台，2028 年增至 20+ 台。',
    why: 'High-NA 实现 1.7 倍特征缩小和 2.9 倍晶体管密度提升。SK Hynix 提前 6 个月安装是 DRAM 制造的里程碑——意味着 High-NA 的应用范围从逻辑扩展到存储。全球仅 12 台意味着产能极度稀缺，分配优先级直接影响各 Fab 的技术路线选择。',
    sources: [
      { name: 'ASML', url: 'https://www.asml.com/en/products/euv-lithography-systems/twinscan-exe-5200b' },
      { name: 'SK Hynix', url: 'https://news.skhynix.com/sk-hynix-introduces-industrys-first-commercial-high-na-euv/' },
      { name: 'TrendForce', url: 'https://www.trendforce.com/news/2026/02/16/news-asmls-high-na-euv-for-2027-28-which-giants-are-betting-big-intel-samsung-sk-hynix-or-tsmc/' },
    ],
  },
  {
    title: 'DRAM 路线图：6F2 → 4F2 → 3D DRAM',
    text: '当前 6F2 单元架构仅能延伸到 1d 节点。下一代 4F2 使用垂直栅晶体管（VG），die 面积缩减 ~30%，SK Hynix 在 VLSI 2025 展示了 4F2 VG 平台路线图。再下一代是 3D DRAM——类似 3D NAND 的垂直堆叠，需要超高深宽比刻蚀（Lam Akara 已为此设计）。',
    why: '4F2 的埋入式位线和垂直沟道是极高深宽比结构，过去沉积工具无法在深沟槽中填充金属（Ru 或 Co），这正是 Lam ALTUS Halo 钼 ALD 和 Akara 刻蚀要解决的核心问题。DRAM 架构每一代变化都意味着设备更新周期，对工艺装备企业是直接的订单驱动力。',
    sources: [
      { name: 'SK Hynix VLSI', url: 'https://www.stocktitan.net/news/HXSCL/sk-hynix-presents-future-dram-technology-roadmap-at-ieee-vlsi-v6pl049dg34b.html' },
      { name: 'Lam 3D DRAM', url: 'https://newsroom.lamresearch.com/learning-from-nand-3d-dram-transition-ai-era' },
      { name: 'SemiEngineering', url: 'https://semiengineering.com/baby-steps-towards-3d-dram/' },
    ],
  },
  {
    title: 'SEMI 全球设备支出展望',
    text: '300mm 设备支出预计 2026 年增长 18% 至 $1,330 亿，2027 年增长 14% 至 $1,510 亿。AI、先进逻辑、存储及先进封装为主要驱动力。台湾 2025 年设备支出暴涨 90%。',
    why: '设备支出增速远超半导体整体增速，说明单位晶体管的工艺复杂度（和设备投入）在加速上升。台湾 90% 的增幅主要来自 TSMC 的 2nm 和 HBM 扩产。',
    sources: [{ name: 'SEMI', url: 'https://www.semi.org/en/semi-press-release/semi-projects-double-digit-growth-in-global-300mm-fab-equipment-spending-for-2026-and-2027' }],
  },
];

// ============================================================
// 05 中国半导体动态
// ============================================================
export const chinaSection = {
  companies: [
    {
      name: '北方华创', ticker: '002371.SZ',
      highlight: 'SEMICON China 2026 发布三款重磅设备，2026 年订单目标 600 亿元，有望跻身全球设备商前五。',
      why: '北方华创的 NMC612H ICP 刻蚀设备已在 SMIC 7nm 产线测试，这是中国刻蚀设备首次进入 sub-10nm 验证阶段。混合键合和 TSV 电镀设备则瞄准先进封装国产化。',
      highlightSources: [{ name: '36Kr', url: 'https://36kr.com/p/3739559184039940' }],
      events: [
        ['新品', '12 英寸 NMC612H ICP 刻蚀、Qomola HPD30 混合键合、Ausip T830 TSV 电镀', [{ name: '新浪财经', url: 'https://finance.sina.com.cn/stock/bxjj/2026-03-30/doc-inhsuknu9640085.shtml' }]],
        ['验证', '刻蚀设备已在 SMIC 7nm 产线测试', [{ name: 'SCMP', url: 'https://www.scmp.com/tech/big-tech/article/3348044/chinas-top-chip-foundry-smic-unveils-action-plan-seizing-new-growth-opportunities' }]],
      ],
    },
    {
      name: '中微公司', ticker: '688012.SH', highlight: null,
      events: [['新品', 'Primo Angnova ICP 刻蚀、Primo Domingo 高选择性刻蚀、Smart RF Match、Preciomo Udx Micro LED MOCVD', [{ name: '快科技', url: 'https://news.mydrivers.com/1/1111/1111592.htm' }]]],
    },
    {
      name: '新凯来', ticker: '未上市（估值近 1000 亿）',
      highlight: 'SEMICON China 发布"名山系列"31 款设备覆盖全工艺链，在手订单超 100 亿元，被称为"中国版 ASML"。',
      why: '新凯来脱胎于华为"星光工程部"，设备号称 100% 自主可控。其产品线覆盖外延（峨眉山系列 EPI）、ALD（阿里山系列）、PVD（普陀山系列）、刻蚀（武夷山系列）、CVD（长白山系列）及量检测，是国产设备全链条替代的标杆案例。',
      highlightSources: [{ name: '东方财富', url: 'https://finance.eastmoney.com/a/202504053365505840.html' }],
      events: [
        ['新品', '"名山系列"31 款设备：覆盖扩散、薄膜沉积、刻蚀、量检测等核心工艺环节，13 类关键量检测产品已在头部晶圆厂量产应用', [{ name: '甲子光年', url: 'https://www.jazzyear.com/article_info.html?id=1598' }]],
        ['光刻', '子公司宇量昇（上海国资+深圳国资）推进干式 DUV 光刻机研发，计划 2026 年量产，采用 SAQP 四次曝光逼近 5nm', [{ name: '新浪财经', url: 'https://finance.sina.com.cn/roll/2025-10-10/doc-inftmnma0975982.shtml' }]],
        ['订单', '在手订单超 100 亿元，2026 年工业制造收入预计达 75 亿元，计划 2027 年净利润达 20 亿后启动 IPO', [{ name: '新浪财经', url: 'https://finance.sina.com.cn/jjxw/2025-09-05/doc-infpkxst7983491.shtml' }]],
      ],
    },
    {
      name: '盛美半导体', ticker: '688082.SH', highlight: null,
      events: [['关注', 'MATCH Act 若通过，盛美的清洗和电镀设备将成为替代 TEL/Lam 的关键选项', []]],
    },
  ],
  outlook: '中国要求新建产能使用不低于 50% 国产设备，目标 2030 年半导体自给率达 80%（现约 33%）。SMIC 发布 2026 行动计划聚焦供应链本土化。2025 年中国 WFE 预计同比 -13%，2026 年回升 +5%。',
  outlookSources: [
    { name: 'Electronics Weekly', url: 'https://www.electronicsweekly.com/news/business/china-makes-new-plan-for-semiconductor-self-sufficiency-2026-03/' },
    { name: 'Astute Group', url: 'https://www.astutegroup.com/news/general/china-accelerates-semiconductor-self-sufficiency-with-mandatory-local-equipment-use/' },
  ],
};

// ============================================================
// 06 学术研究
// ============================================================
export const academic = [
  {
    title: 'Imec 120 层 Si/SiGe 外延堆叠突破 3D DRAM 瓶颈',
    text: 'Imec 联合根特大学在 300mm 晶圆上成功堆叠 120 层交替 Si/SiGe 双层（共 241 子层），发表于 Journal of Applied Physics。',
    why: '3D DRAM 要求在单片晶圆上垂直堆叠超过 100 层半导体薄膜并保持厚度均匀性——这是前所未有的工艺挑战。此突破证明外延生长路线可行，直接影响 TEL EVAROS 和 AMAT Centura 等批量热处理/外延设备的需求。',
    sources: [
      { name: "Tom's Hardware", url: 'https://www.tomshardware.com/tech-industry/next-generation-3d-dram-approaches-reality-as-scientists-achieve-120-layer-stack-using-advanced-deposition-techniques' },
    ],
  },
  {
    title: 'Nature Electronics: 碳纳米管晶体管突破 1 THz',
    text: '基于对齐碳纳米管薄膜的 MOSFET 实现截止频率超 1 THz，栅极长度 80nm，载流子迁移率超 3,000 cm²/V·s。',
    why: '这是后硅时代高频器件的里程碑。碳纳米管的载流子迁移率远超硅（~450 cm²/V·s），为太赫兹通信和超高速计算提供了新的材料路径。',
    sources: [{ name: 'Nature Electronics', url: 'https://www.nature.com/articles/s41928-025-01463-6' }],
  },
  {
    title: 'Imec + Diraq: 工厂制造硅量子比特达 99%+ 保真度',
    text: '在标准半导体产线上制造的硅量子比特一致性超过 99% 精度，发表于 Nature。',
    why: '证明量子计算芯片可以复用现有 CMOS 产线制造——这意味着未来量子计算不需要独立的制造体系，现有设备投资可以复用。',
    sources: [{ name: 'Nature', url: 'https://www.nature.com/articles/s41586-025-09531-9' }],
  },
  {
    title: 'CFET（互补 FET）路线图：GAA 之后的下一代晶体管',
    text: 'Imec 推进 CFET 单片集成与顺序集成方案。全芯片评估显示 CFET 相比 Nanosheet FET 可实现面积缩减 ~55%、功耗降低 ~29%。预计 A7 节点（2030 年后）开始量产。',
    why: 'CFET 将 NFET 和 PFET 垂直堆叠（不是并排），代表晶体管架构从 2D 到真正 3D 的跨越。单片集成方案需要极高深宽比刻蚀，顺序集成方案需要晶圆键合——两条路线都意味着新的设备需求。GAA 预计至少延续 3 代技术节点才会过渡到 CFET。',
    sources: [
      { name: 'Imec', url: 'https://www.imec-int.com/en/articles/imec-puts-complementary-fet-cfet-logic-technology-roadmap' },
      { name: 'Lam CFET', url: 'https://newsroom.lamresearch.com/understanding-cfets-transistor-architecture?blog=true' },
    ],
  },
];

// ============================================================
// 07 AI 与智能化
// ============================================================
export const aiSmart = [
  {
    title: 'Intel 将 AI 嵌入晶圆厂核心流程',
    text: 'AI 缺陷检测精度已超人工检测员，NVIDIA NV-DINOv2 视觉模型达 98.5% die 级缺陷分类精度。先进逻辑晶圆厂每 1% 良率提升 ≈ $1.5 亿利润。',
    why: '对工艺工程师意味着：AI 不再只是辅助分析，而是直接嵌入掩模检测、良率预测和实时推理。KLA 和 ASML 的设备数据流将成为 AI 训练的核心数据源。',
    sources: [
      { name: 'EE Times', url: 'https://www.eetimes.com/from-defect-images-to-die-prediction-how-intel-is-scaling-ai-in-advanced-manufacturing/' },
    ],
  },
  {
    title: 'EDA 进入 Agent 时代',
    text: 'Cadence 发布 ChipStack AI Agent（10x 生产力），Synopsys 推出 AgentEngineer。NVIDIA 投资 $20 亿入股 Synopsys。新锐 Cognichip 融资 $6,000 万（Intel CEO 加入董事会）。',
    why: 'Agent 化的 EDA 工具意味着芯片设计从"工具操作"转向"意图描述"——工程师描述想要什么，AI 自动生成 RTL、测试平台和验证方案。这将重新定义半导体设计人才的技能要求。',
    sources: [
      { name: 'The Register', url: 'https://www.theregister.com/2026/02/10/cadences_agentic_chip_design_tool/' },
      { name: 'TechCrunch', url: 'https://techcrunch.com/2026/04/01/cognichip-wants-ai-to-design-the-chips-that-power-ai-and-just-raised-60m-to-try/' },
    ],
  },
  {
    title: 'HBM4 量产启动，HBM 演进路线图',
    text: 'Samsung 出货首批商用 HBM4（11.7 Gbps，高于标准 46%），接口翻倍至 2048-bit，带宽 2-3.3 TB/s。HBM4E 预计 2027 年，可能堆叠 20 层。HBM4 使用混合键合取代焊球，降低热阻。',
    why: 'HBM 是 AI 加速器的性能瓶颈。每一代 HBM 演进都拉动混合键合设备（TEL Synapse、Besi）、检测设备（KLA Kronos）和晶圆减薄设备（TEL Ulucus）的需求。2026 年 HBM 销售预计翻三倍。',
    sources: [
      { name: 'Samsung', url: 'https://news.samsung.com/global/samsung-ships-industry-first-commercial-hbm4-with-ultimate-performance-for-ai-computing' },
      { name: "Tom's Hardware", url: 'https://www.tomshardware.com/tech-industry/semiconductors/hbm-roadmaps-for-micron-samsung-and-sk-hynix-to-hbm4-and-beyond' },
    ],
  },
  {
    title: 'RISC-V 进入量产汽车',
    text: 'Bosch、Infineon、Nordic、NXP、Qualcomm 五家成立德国合资公司聚焦车规 RISC-V。零跑 LingXin01 SoC 用于 ADAS；长城紫荆 M100 MCU 已搭载新车型。',
    why: '车规芯片转向 RISC-V 代表汽车行业对 Arm 授权依赖的解绑趋势。对工艺装备的影响：车规芯片以成熟制程为主（28-40nm），利好 UMC、GF 等成熟代工厂的设备投资。',
    sources: [
      { name: 'EE Times', url: 'https://www.eetimes.com/automotive-industry-charts-new-course-with-risc-v/' },
    ],
  },
];

export const aiSmartOutro = {
  label: 'Capital Flow',
  text: '四大超算巨头 2026 年 AI 基础设施资本支出计划达 $6,500 亿，同比增 71%。Intel 加入 Musk 的 Terafab 项目（$250 亿），贡献 18A 工艺。OpenAI 与 Broadcom 合作开发 10GW 定制 AI 加速器。TSMC CoWoS 产能：2024 ~40K WPM → 2025 75K → 2026 目标 120-130K，NVIDIA 预订 50%+。',
  sources: [
    { name: 'Deloitte', url: 'https://www.deloitte.com/us/en/insights/industry/technology/technology-media-telecom-outlooks/semiconductor-industry-outlook.html' },
    { name: 'TNW', url: 'https://thenextweb.com/news/intel-terafab-elon-musk-foundry-partnership' },
  ],
};

// ============================================================
// 08 产品与体验设计
// ============================================================
export const designTrends = [
  {
    title: 'Cadence ChipStack AI Super Agent：EDA 从菜单交互走向意图对话',
    text: 'Cadence 发布 ChipStack AI Super Agent（2 月 10 日），让工程师用自然语言描述意图，由多智能体自动生成 RTL、测试平台和验证方案。早期客户 NVIDIA、Qualcomm、Altera、Tenstorrent 反馈生产力提升 10x。',
    why: '这是 EDA 工具 UX 从"工具操作"到"意图对话"的历史性转向。对工艺装备行业的间接影响：更快的芯片设计周期意味着更频繁的 tape-out，拉动 Fab 产能需求。',
    sources: [
      { name: 'Cadence', url: 'https://www.cadence.com/en_US/home/company/newsroom/press-releases/pr/2026/cadence-unleashes-chipstack-ai-super-agent-pioneering-a-new.html' },
    ],
  },
  {
    title: 'Siemens Digital Twin Composer：HMI 从仪表盘走向虚拟孪生',
    text: 'Siemens 在 CES 2026 发布 Digital Twin Composer，基于 NVIDIA Omniverse 将 2D/3D 数字孪生数据与实时遥测统一到一个场景中。"一个模型贯穿设计、仿真、运行"。',
    why: '晶圆厂操作员 HMI 正在从传统平面仪表盘重塑为可导航的 3D 虚拟孪生空间。这对 TEL Epsira DX 这类数字化平台是直接竞争和合作的参照。',
    sources: [
      { name: 'Siemens', url: 'https://news.siemens.com/en-us/digital-twin-composer-ces-2026/' },
    ],
  },
  {
    title: 'NVIDIA Vera Rubin：整机架即产品',
    text: 'CES 和 GTC 2026 发布 Vera Rubin 平台——6 颗芯片协同设计为统一 AI 超级计算机机架，标志着 AI 硬件从"芯片级产品"到"机架级产品"的设计语言转变。',
    why: '机架级设计意味着对先进封装（CoWoS-L）、HBM 堆叠和高密度互连的需求进一步升级。一个 Vera Rubin 机架消耗的封装产能可能相当于数千颗传统芯片。',
    sources: [
      { name: 'NVIDIA', url: 'https://nvidianews.nvidia.com/news/rubin-platform-ai-supercomputer' },
    ],
  },
];

export const designTrendsOutro = {
  label: 'UX Shift',
  text: 'NVIDIA 宣布与 TSMC、SK 海力士、三星、联发科等工业软件巨头合作，将 Agentic AI 嵌入设计、工程和制造软件。半导体制造 UX 迎来新的交互底层——从人工操作复杂工作流到 AI Agent 自主编排。TEL Epsira DX 和 KLA DefectWise AI 是这一趋势在设备端的落地。',
  sources: [
    { name: 'NVIDIA IR', url: 'https://investor.nvidia.com/news/press-release-details/2026/NVIDIA-and-Global-Industrial-Software-Giants-Bring-Design-Engineering-and-Manufacturing-Into-the-AI-Era/default.aspx' },
  ],
};

// ============================================================
// 09 政策与地缘
// ============================================================
export const policy = [
  {
    tag: 'U.S. Congress',
    title: 'MATCH Act：封堵 DUV 对华出口',
    text: '4 月 2 日众议院推出 MATCH Act，目标收紧 ASML DUV 光刻机对华出口，施压荷兰、日本盟友全面对齐美国管制标准。JPMorgan 估计若实施，ASML EPS 可能下降 10%。',
    why: '影响分三层：(1) 设备获取——中国 Fab 可能无法获得 DUV immersion（ArFi）以上的光刻设备；(2) 国产替代加速——北方华创、中微、盛美的订单可能因此增长；(3) 全球供应链重组——设备厂商需要重新规划产能分配和客户结构。',
    sources: [
      { name: 'Bloomberg', url: 'https://www.bloomberg.com/news/articles/2026-04-02/us-lawmakers-propose-crackdown-on-sales-of-chip-tools-to-china' },
    ],
  },
  {
    tag: 'U.S. Tariff',
    title: 'Section 232 半导体关税生效',
    text: '1 月 15 日起对特定先进芯片征收 25% 关税（涵盖 Nvidia H200、AMD MI325X 等），但豁免数据中心、研发等场景。7 月 1 日将评估是否扩大范围。',
    why: '关税直接影响 AI 加速器的终端部署成本，间接影响对 HBM 和 CoWoS 封装产能的需求节奏。',
    sources: [{ name: 'White & Case', url: 'https://www.whitecase.com/insight-alert/president-trump-orders-narrowly-targeted-25-section-232-tariff-certain-advanced' }],
  },
  {
    tag: 'CHIPS Act',
    title: 'CHIPS Act 税收优惠面临到期',
    text: 'TSMC 宣布追加 $1,000 亿美国投资。但 Section 48D 先进制造投资税收抵免将于 2026 年到期，续期前景不明。',
    why: '如果 48D 不续期，美国本土 Fab 建设的经济性将显著下降，可能影响 Intel、TSMC、Samsung 在美扩产计划的节奏——进而影响设备采购时间线。',
    sources: [
      { name: 'Conference Board', url: 'https://www.conference-board.org/research/ced-policy-backgrounders/the-future-of-the-CHIPS-and-Science-Act' },
    ],
  },
];

// ============================================================
// 10 产业格局
// ============================================================
export const landscape = {
  text: 'TSMC 亚利桑那规模扩大至最多 12 座晶圆厂 + 4 座先进封装厂；台湾同步推进 10 座新厂。全球 2025 年新开工 18 个晶圆厂项目（3 座 200mm + 15 座 300mm），美洲和日本各 4 个领跑。CoWoS 产能：2024 ~40K WPM → 2025 75K → 2026 目标 120-130K，NVIDIA 预订 50%+。Intel 正与 Google、Amazon 洽谈先进封装代工服务。',
  sources: [
    { name: 'TrendForce', url: 'https://www.trendforce.com/news/2026/02/23/news-tsmc-speeds-up-expansion-in-taiwan-up-to-10-fabs-reportedly-under-construction-or-starting-in-2026/' },
    { name: 'SEMI', url: 'https://www.semi.org/en/semi-press-release/eighteen-new-semiconductor-fabs-to-start-construction-in-2025-semi-reports' },
  ],
};

// ============================================================
// 11 值得一读
// ============================================================
export const reading = [
  {
    title: 'Lam Research: Everything About Akara',
    source: 'Lam Newsroom · Feb 2025',
    desc: 'Akara 导体刻蚀系统的完整技术解析——DirectDrive 固态等离子体源、TEMPO 脉冲、SNAP 离子控制。理解下一代刻蚀技术的必读材料。',
    url: 'https://newsroom.lamresearch.com/everything-about-akara',
  },
  {
    title: 'SEMI: Global 300mm Fab Equipment Spending to Reach $151B by 2027',
    source: 'SEMI.org · Apr 8, 2026',
    desc: 'SEMI 最新预测报告，详细拆解各地区、各环节设备投资趋势，是理解行业增长逻辑的必读材料。',
    url: 'https://www.semi.org/en/semi-press-release/semi-projects-double-digit-growth-in-global-300mm-fab-equipment-spending-for-2026-and-2027',
  },
  {
    title: 'Backside Power Delivery Nears Production',
    source: 'SemiEngineering · Mar 2026',
    desc: '深度解析背面供电技术的量产挑战，涵盖 TSMC、Intel、Samsung 的不同技术路线和对应的设备需求。',
    url: 'https://semiengineering.com/backside-power-delivery-nears-production/',
  },
  {
    title: 'MATCH Act: What It Means for Semiconductor Equipment Exports',
    source: 'Bloomberg · Apr 2, 2026',
    desc: '法案全文解读与影响分析，重点关注对 ASML DUV 业务及日荷盟友的潜在连锁反应。',
    url: 'https://www.bloomberg.com/news/articles/2026-04-02/us-lawmakers-propose-crackdown-on-sales-of-chip-tools-to-china',
  },
];

// ============================================================
// 12 关键日程
// ============================================================
export const calendar = [
  { date: 'Apr 15', event: 'ASML Q1 2026 财报发布' },
  { date: 'Apr 22', event: 'Lam Research Q3 FY2026 财报发布' },
  { date: 'Apr 22', event: 'ASML 年度股东大会' },
  { date: 'Apr 29', event: 'KLA Q3 FY2026 财报发布' },
  { date: 'Apr 30', event: 'Tokyo Electron 全年财报发布' },
  { date: 'Jun 14', event: 'VLSI Symposium 2026 檀香山' },
  { date: 'H2 2026', event: 'TSMC A16（GAA + 背面供电）投产' },
  { date: 'Q4 2026', event: 'Imec EXE:5200 High-NA EUV 验证完成' },
  { date: 'Q4 2026', event: 'Albany NanoTech High-NA 中心 first light' },
];

// ============================================================
// 13 综合判断（V2 新增）
// ============================================================
export const synthesis = {
  title: '综合判断',
  subtitle: 'Synthesis',
  techLines: [
    {
      theme: '埃米级制造进入量产',
      insight: 'GAA 纳米片从研发走向 HVM（TSMC N2 已量产、A16 H2 2026），每一代节点对选择性刻蚀（Lam）、原子层沉积（AMAT/TEL）和量测（KLA）的需求成倍增长。Lam 在 sub-5nm 选择性刻蚀市场占 ~80% 份额。',
    },
    {
      theme: 'DRAM 架构换代推动设备更新',
      insight: 'High-NA EUV 首次进入 DRAM（SK Hynix M16），6F2→4F2 架构转换需要全新的高深宽比刻蚀和金属填充设备。Lam Akara + ALTUS Halo 精准卡位。3D DRAM 长期看是下一个设备超级周期。',
    },
    {
      theme: '先进封装成为第三增长极',
      insight: 'CoWoS 产能 3 年翻 3 倍（40K→120K WPM），混合键合设备 TAM 从 ¥100B 增至 ¥300B（TEL 预测）。KLA Lumina 覆盖玻璃芯基板——Intel 计划 2026-27 年商用。封装不再是后端，而是性能的关键差异化环节。',
    },
  ],
  processImpact: [
    { area: '光刻', impact: 'High-NA 部署加速（DRAM 首用），配套涂布/显影（TEL DICE）和 EUV 辅助处理（TEL Acrevia）需求同步增长', priority: 5 },
    { area: '刻蚀', impact: 'GAA 选择性刻蚀 + 4F2 DRAM 高深宽比刻蚀双驱动，Lam Akara 已获量产订单', priority: 5 },
    { area: '沉积', impact: '钼替代钨（Lam ALTUS Halo）+ GAA 栅极堆栈（AMAT Trillium）+ 批量 ALD（TEL EVAROS）', priority: 5 },
    { area: '量测/检测', impact: 'GAA 增加 30-50% 量测层，先进封装需要新型检测（KLA Lumina/Kronos），X 射线量测（AXION T2000）', priority: 4 },
    { area: '先进封装', impact: '混合键合（TEL Synapse）、晶圆减薄/剥离（TEL Ulucus）、CoWoS 产能扩张', priority: 4 },
    { area: '背面供电', impact: 'TSMC A16 H2 2026 投产，需要 nano-TSV 刻蚀、极薄晶圆处理、新型对准设备', priority: 3 },
  ],
};
